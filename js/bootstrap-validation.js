...
!function($) {
	...
	$.fn.validation.defaults = {
		validRules : [
			...
			{name: 'numberhyphen', validate: function(value) {return (!/^[0-9][\-\d]*$/.test(value));}, defaultMsg: '只能输入数字和短横线。'},
			...
			//{name: 'date', validate: function(value) {return (!/^\d{4}\-\d{2}\-\d{2}$/.test(value));}, defaultMsg: '请输入日期。'},
			{name: 'nospace', validate: function(value) {return (!/^[0-9a-z\_A-Z\u4e00-\u9fff]+$/.test(value));}, defaultMsg: '请勿包含空格。'},
			{name: 'mobile', validate: function(value) {return (!/^(1[3,5,8,7]{1}[\d]{9})|(((400)-(\d{3})-(\d{4}))|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{3,7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)$/.test(value));}, defaultMsg: '请输入手机号。'},
		]
	};
